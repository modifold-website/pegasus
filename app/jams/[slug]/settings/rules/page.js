...
import ModJamRulesSettings from "@/components/mod-jams/settings/ModJamRulesSettings";
...
export default async function ModJamRulesSettingsPage({ params }) {
	const { slug } = await params;
	const cookieStore = await cookies();
	const authToken = cookieStore.get("authToken")?.value;
	const data = await getJam(slug, authToken);
	...
	return <ModJamRulesSettings authToken={authToken} jam={data.mod_jam} />;
}
...
import ModJamJurySettings from "@/components/mod-jams/settings/ModJamJurySettings";
...
export default async function ModJamJurySettingsPage({ params }) {
	const { slug } = await params;
	const cookieStore = await cookies();
	const authToken = cookieStore.get("authToken")?.value;
	const data = await getJam(slug, authToken);
	...
	return <ModJamJurySettings authToken={authToken} jam={data.mod_jam} initialJury={data.jury || []} />;
}
...
import ModJamGeneralSettings from "@/components/mod-jams/settings/ModJamGeneralSettings";
...
export default async function ModJamSettingsPage({ params }) {
	const { slug } = await params;
	const cookieStore = await cookies();
	const authToken = cookieStore.get("authToken")?.value;
	const data = await getJam(slug, authToken);
	...
	return <ModJamGeneralSettings authToken={authToken} jam={data.mod_jam} />;
}
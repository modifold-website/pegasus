...
import ModJamLinksSettings from "@/components/mod-jams/settings/ModJamLinksSettings";
...
export default async function ModJamLinksSettingsPage({ params }) {
	const { slug } = await params;
	const cookieStore = await cookies();
	const authToken = cookieStore.get("authToken")?.value;
	const data = await getJam(slug, authToken);
	...
	return <ModJamLinksSettings authToken={authToken} jam={data.mod_jam} />;
}